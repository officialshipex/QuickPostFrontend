    ...
    <AdminLayout>
      <div className="flex flex-col h-[calc(100vh-72px)] -m-4 md:-m-6 bg-white">
        <div className="bg-white relative z-50 shrink-0">
          ...
          {/* Page Header */}
          <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[#E2E8F0] bg-white flex justify-between items-center">
            <div>
              <h1 className="text-[18px] md:text-[28px] font-bold text-[#0F172A] tracking-tight flex items-center gap-2">
                <Megaphone className="w-5 h-5 md:w-6 md:h-6 text-[#00A86B]" />
                <span className="hidden md:inline">Important Announcements</span>
                <span className="md:hidden">Announcements</span>
              </h1>
            </div>
            <button
              onClick={openModal}
              className="hidden md:flex h-9 px-4 rounded-full bg-[#00A86B] text-white text-[11px] font-bold items-center gap-1.5 shadow-sm hover:bg-[#009B63] transition-colors cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5" /> Create Announcement
            </button>
            <button
              onClick={openModal}
              className="md:hidden w-9 h-9 rounded-full bg-[#00A86B] text-white flex items-center justify-center shadow-sm active:bg-[#009B63] transition-colors shrink-0"
          ...
          {/* Filter Row (desktop) */}
          <div className="hidden md:flex py-3 px-6 border-b border-[#CBD5F5] flex-wrap gap-3 items-center bg-white">
            <div className="relative">
                      ...
                      <div className="pt-6 px-2.5 pb-2.5">
                        ...
                        <div className="flex items-center justify-between mb-1.5 gap-2">
                          <div className="min-w-0">
                            <span className="text-[12px] font-semibold text-[#0F172A]">{fmtDate(ann.createdAt)}</span>
                            <span className="text-[12px] font-normal text-[#94A3B8] ml-1.5">{fmtTime(ann.createdAt)}</span>
                          </div>
                        ...
                        {/* Message */}
                        <p className="text-[12px] font-normal text-[#1E293B] leading-relaxed mb-2 line-clamp-3">{ann.message}</p>

                        {/* Tags */}
                        <div className="flex items-center gap-1.5 flex-wrap mb-2.5">
                          <span className={`px-2 py-0.5 rounded-full border text-[10px] font-semibold ${ann.disableType === 'manual' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-purple-50 text-purple-700 border-purple-200'}`}>
                            {ann.disableType === 'manual' ? 'Manual' : 'Automated'}
                          </span>
                          <span className="px-2 py-0.5 rounded-full border border-[#DCFCE7] text-[10px] font-semibold bg-[#F0FDF4] text-[#00A86B]">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => openEditModal(ann)}
                            className="flex-1 h-8 rounded-xl border border-[#E2E8F0] text-[#475569] text-[12px] font-bold flex items-center justify-center gap-1.5 hover:bg-[#F8FAFC] transition-colors"
                          >
                            <Edit2 className="w-3.5 h-3.5" /> Edit
                          </button>
                          <button
                            onClick={() => handleDelete(ann._id)}
                            className="w-8 h-8 rounded-full border border-[#E2E8F0] text-[#64748B] bg-white flex items-center justify-center hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shrink-0"
                          >
      ...
      {/* Create / Edit Modal */}
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4">
            ...
            <div className="relative w-full max-w-lg h-full md:h-auto bg-white rounded-none md:rounded-2xl shadow-2xl overflow-hidden animate-fade-in max-h-full md:max-h-[90vh] flex flex-col">
