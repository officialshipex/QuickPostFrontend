...
export function NdrStatusModal({ isOpen, setIsOpen, ndrHistory }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-[200]" tabIndex={-1} ref={modalRef} onClick={onBackdropClick}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col border border-[#E2E8F0] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E2E8F0] bg-[#F8FAFC] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#64748B]">Detailed tracking and action history for undelivered reports</p>
            </div>
          </div>
          <button onClick={() => setIsOpen(false)} className="w-8 h-8 rounded-lg border border-[#E2E8F0] flex items-center justify-center text-[#64748B] hover:bg-white transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {tabKeys.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all border ${activeTab === tab ? 'bg-[#00A86B] text-white border-[#00A86B]' : 'bg-white text-[#475569] border-[#E2E8F0] hover:bg-[#F8FAFC]'}`}>
                {TAB_LABELS[tab]}
              </button>
            ))}
          </div>
        ...
        {/* Footer */}
        <div className="flex justify-end px-5 py-4 border-t border-[#E2E8F0] bg-[#F8FAFC] shrink-0">
          <button onClick={() => setIsOpen(false)} className="px-4 py-2 bg-white border border-[#E2E8F0] text-[#475569] text-[12px] font-bold rounded-lg hover:bg-[#F8FAFC] transition-all">Close</button>
        </div>
      </div>
    </div>,
    document.body
