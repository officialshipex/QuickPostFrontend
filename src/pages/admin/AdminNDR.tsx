...
import {
  ...
  User, Settings, MapPin, X, Truck, Loader2,
...
export function AdminNDR() {
  ...
  const fetchOrders = useCallback(async (pg = page) => {
    ...
    try {
      ...
      const params: Record<string, any> = { page: pg, limit: ITEMS_PER_PAGE, ...tabParams };
      ...
      if (res.data?.courierServices?.length) setCourierOptions(res.data.courierServices.map((c: any) => ({ label: c.name || c, value: c.name || c })));
      if (res.data?.pickupLocations?.length)  setPickupOptions(res.data.pickupLocations.map((p: any) => ({ label: p.name || p, value: p.name || p })));
    } catch (err) {
  ...
  }, [activeTab, page, orderId, awbNumber, selectedCouriers, selectedPaymentTypes, selectedPickups,
      ...
      <div className="flex flex-col h-[calc(100vh-72px)] -m-4 md:-m-6 bg-white">
                  ...
                  {orders.length === 0 ? (
                    ...
                    <tr key={order._id} className="border-b border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors group">
                      ...
                      <td className="p-3 min-w-[130px]">
                        ...
                        {order.lastNdrReason && (
                          <div className="text-[10px] text-[#64748B] mt-0.5 truncate max-w-[120px]">{order.lastNdrReason}</div>
                        )}
                      </td>
        ...
        {hoveredPickup && (
          <div className="fixed z-[9999] pointer-events-none bg-[#0F172A] text-white text-[10px] p-3 rounded-xl shadow-xl w-64"
            ...
            {hoveredPickup.address && <div className="text-slate-300 font-medium leading-relaxed border-t border-white/10 pt-1.5">{hoveredPickup.address}</div>}
