...
export function AdminWeightDiscrepancy() {
  ...
  }, [activeTab, page, limit, selectedCouriers, dateStart, dateEnd, searchInput, searchBy, userMongoId, isAdminView, currentUserId]);
          ...
          {totalPages > 0 && (
            <div className="p-4 border-t border-[#E2E8F0] flex items-center justify-between shrink-0">
              <div className="text-xs text-[#64748B]">
                Showing <span className="font-bold text-[#0F172A]">{startIdx}</span>–<span className="font-bold text-[#0F172A]">{endIdx}</span> of <span className="font-bold text-[#0F172A]">{total}</span>
              </div>
                  ...
                  return (
                    <button key={pg} onClick={() => setPage(pg)}
                      className={`w-8 h-8 rounded text-xs font-medium flex items-center justify-center transition-colors ${
                        page === pg ? 'bg-[#00A86B] text-white border border-[#00A86B]' : 'border border-[#E2E8F0] text-[#475569] hover:bg-[#F8FAFC]'
                      }`}>
                      {pg}
                    </button>
                  );
