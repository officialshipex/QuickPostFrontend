...
import { Wallet, Building2, UserCircle, FileText, Banknote, Scale, Archive, ArrowLeft, Lock } from 'lucide-react';
...
export function AdminTransferCOD() {
  ...
  // Reset adjustMode when fresh balance is loaded
  ...
  // Hold logic — which remittances are held due to holdAmount
  const holdResolved = useMemo(() => {
    if (bypassHold || !holdAmount || holdAmount <= 0)
      return { heldIds: [] as string[], heldAmount: 0 };
    ...
    if (single) {
      return { heldIds: [String(single.remittanceId || single._id)], heldAmount: single.remittanceAmount };
    }
    ...
    return { heldIds: chosen.map((c) => String(c.remittanceId || c._id)), heldAmount: total };
  }, [holdAmount, remittanceEntries, bypassHold]);

  // Wallet top-up — which remittances cover the full negative balance
  const walletTopUp = useMemo(() => {
    ...
    if (single) {
      return { needed, topUpIds: [String(single.remittanceId || single._id)], topUpAmount: single.remittanceAmount };
    }
    ...
    return { needed, topUpIds: chosen.map((s) => String(s.remittanceId || s._id)), topUpAmount: sum };
  }, [balance, remittanceEntries, holdResolved]);

  // Negative-only top-up — partial adjustment of a single remittance
  ...
  // Frozen — top-up candidates that are neither paid nor adjusted (only when adjustMode is null)
  const frozenIds = useMemo(() => {
    if (adjustMode === null) return [...walletTopUp.topUpIds];
    return [] as string[];
  }, [adjustMode, walletTopUp]);

  // Payable — entries actually paid to the client after hold/frozen/topup exclusions
  const payableInfo = useMemo(() => {
    ...
    const total = payable.reduce((sum, r) => {
                ...
                {balance < 0 && (
                  <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-2xl">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-[13px] bg-white border border-red-200 rounded-xl mb-2"
                    />
                        ...
                        <tbody className="divide-y divide-[#E2E8F0] bg-white">
                          {remittanceEntries.length === 0 ? (
                            ...
                            if (isHeld) {
                              ...
                              noteEl = <span className="inline-block px-3 py-1 rounded-lg text-[11px] font-bold text-orange-500 bg-orange-50 border border-orange-200">Partial TopUp (&#8377;{walletNegativeOnly.needed.toFixed(2)})</span>;
                            } else {
