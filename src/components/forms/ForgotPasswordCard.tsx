...
export function ForgotPasswordCard() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await apiClient.post('/external/forgetPassword', {
        email: data.email,
  ...
  if (status === 'success') {
    return (
      <div className="bg-white p-8 md:p-10 rounded-2xl shadow-2xl shadow-black/10 text-center">
        ...
        <h2 className="text-2xl font-bold text-[#0F172A] mb-2">Password reset successfully</h2>
  ...
  return (
    <div className="bg-white p-8 md:p-10 rounded-2xl shadow-2xl shadow-black/10">
      <div className="text-center mb-8">
        ...
        <p className="text-[#64748B] text-sm">Enter your email and set a new password.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          <label className="block text-xs font-semibold text-[#475569] mb-1.5">New Password<span className="text-red-500">*</span></label>
          <Input
            type="password"
